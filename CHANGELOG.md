...
- Claude Code 연결을 그대로 유지하면서 OMP와 Codex CLI를 선택해 함께 연결할 수 있습니다. `setup --client`, `connect`, `disconnect`는 실제 도구 경로별 소유 상태를 기록하고, 바뀌지 않은 관리 파일만 안전하게 되돌립니다. OMP와 Codex의 새 세션에서는 지식 원문을 훑거나 설정을 쓰지 않는 짧은 시작 확인을 실행하며, 실제 도구 실행 여부와 index 최신 여부는 따로 구분해 안내합니다.

### 수정

- 여러 도구 연결 중 파일이나 선택 상태가 동시에 바뀌면 이번 작업만 조건부로 되돌리고, 기존 Claude 연결과 명시한 Claude 설정 경로를 계속 진단합니다. 연결 해제 뒤 소유하지 않거나 수정된 발견 파일·복수 Codex hook이 남으면 해제 완료로 표시하지 않으며, dry-run과 변경 거절은 자동 업데이트 확인을 실행하지 않습니다.
- 연결 적용 실패 시 다른 작업이 만든 파일이나 수정한 내용을 보존하고, 복구를 끝내지 못하면 `CONNECTION_ROLLBACK_INCOMPLETE`와 상태 확인 방법을 안내합니다. Codex를 먼저 해제해도 공유 지침이 남아 있으면 OMP를 완전히 해제된 것으로 표시하지 않습니다. 대화형 setup과 Claude 연결을 거절했을 때도 업데이트 요청이나 캐시를 만들지 않습니다.
